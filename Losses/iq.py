...
class IQLoss(nn.Module):
    def __init__(self, grad_pen=False, method="value_expert", div="chi", gamma=0, lambda_gp=0, size_average=True):
        super(IQLoss, self).__init__()
        self.grad_pen = grad_pen
        self.method = method
        self.div = div
        self.alpha = gamma  # Rename hyperparam
    ...
    def iq_loss(self, logits, targets):
        ...
        if self.div == "hellinger":
            # Hellinger such that logits=0 => rewards = 0, and min rewards = -1
            reward = F.softplus(logits + math.log(math.e - 1)) - 1 + 1e-5
        elif self.div == "js":
        ...
        if self.method == "value_expert":
            ...
            # print("vl shape", value_loss.shape)
            # print("l shape", loss.shape)
            loss += value_loss
            ...
            value_loss = torch.logsumexp(reward, dim=1) + torch.logsumexp(expert_logits, dim=1)
            loss += value_loss
            loss_dict['value_loss'] = value_loss.mean().item()
        else:
        ...
        if self.grad_pen:
            ...
            gp_loss = agent.critic_net.grad_pen(reward,
                                                self.lambda_gp)
